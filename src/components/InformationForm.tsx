...
import { NewInformation, ApiError } from '../types';
...
const schema = yup.object({
  ...
  localizacaoAvistada: yup
    .string()
    .optional(),
  telefoneContato: yup
    .string()
    .optional()
    .matches(/^(\(\d{2}\)\s\d{4,5}-\d{4})?$/, 'Formato do telefone inválido'),
});
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const informationData: NewInformation = {
        ...
        localizacaoAvistada: data.localizacaoAvistada || undefined,
